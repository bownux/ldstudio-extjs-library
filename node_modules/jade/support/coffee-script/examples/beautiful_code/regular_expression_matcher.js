(function() {
  ...
  match = function(regexp, text) {
    if (regexp[0] === '^') return match_here(regexp.slice(1), text);
    while (text) {
  ...
  match_here = function(regexp, text) {
    ...
    if (next === '*') return match_star(cur, regexp.slice(2), text);
    if (cur === '$' && !next) return text.length === 0;
  ...
  match_star = function(c, regexp, text) {
    ...
    while (true) {
      if (match_here(regexp, text)) return true;
      if (!(text && (text[0] === c || c === '.'))) return false;
