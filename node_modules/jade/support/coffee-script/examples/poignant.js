(function() {
  ...
  Creature = {
    hit: function(damage) {
      ...
      this.life -= damage;
      if (this.life <= 0) return puts("[" + this.name + " has died.]");
    },
    fight: function(enemy, weapon) {
      var enemy_hit, your_hit;
      if (this.life <= 0) return puts("[" + this.name + "is too dead to fight!]");
  ...
  wipe_mutterings_from = function(sentence) {
    ...
    if (!sentence.indexOf) throw new Error("cannot wipe mutterings");
    _results = [];
