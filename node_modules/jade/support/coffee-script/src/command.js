(function() {
  var BANNER, CoffeeScript, EventEmitter, SWITCHES, compileOptions, compileScript, compileScripts, compileStdio, exec, fs, helpers, lint, optionParser, optparse, opts, parseOptions, path, printTokens, sources, spawn, usage, version, watch, writeJs, _ref;
  ...
  exports.run = function() {
    ...
    if (opts.eval) return compileScript('console', sources[0]);
    ...
    if (opts.run) flags = sources.splice(1).concat(flags);
  ...
  compileScript = function(file, input, base) {
    ...
    try {
      ...
      if (o.watch) return puts(err.message);
      error(err.stack);
  ...
  writeJs = function(source, js, base) {
    ...
    compile = function() {
      ...
      return fs.writeFile(jsPath, js, function(err) {
        if (opts.compile && opts.watch) return puts("Compiled " + source);
      });
    };
